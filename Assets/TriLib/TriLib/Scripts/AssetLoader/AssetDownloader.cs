...
namespace TriLib
{
    ...
    public class AssetDownloader : MonoBehaviour
    {
        ...
        //    }
        //    var centeredRect = new Rect(Screen.resolutions[0].width / 2f - 100f, Screen.currentResolution.height / 2f - 25f, 200f, 50f);
        //    print(Screen.resolutions[0].width);
        //    GUI.skin.label.normal.textColor = Color.white;
        //    GUI.Label(centeredRect, string.Format("正在加载模型...{0:P2}", Progress), _centeredStyle);
        //}
        private void Update()
        {
            if (!ShowProgress || !HasStarted || IsDone)
            ...
            progressText.text = string.Format("正在加载模型... {0:P2}", Progress);
        }
                ...
                try
                {
                    ...
                    else
                    {
                        using (var assetLoader = new AssetLoader())
                        {
                            ...
                            {
                                GameObject go;
                                go = assetLoader.LoadFromMemoryWithTextures(data, assetExtension, options, wrapperGameObject, null, null, null, progressCallback);
                                Debug.Log(go);
